...
// Manchester             1 1 1 0  1 1 0 1 => 0xED (237)
    decodeManchester2(pBits){
        // copy bits to array
        //var bits = Dekoder.bytesToBits(pBytes)
...
function dekoder_selftest(){
    ...
    var t1 = Dekoder.debugToTrace(trace) 
    if ( t1 == null) {
    ...
    dek.debug("> " + Dekoder.toHexString(nextbytes) )
    dek.debug("> Pos" + dek.getPosition() )

    var nextbytes = dek.readBytes(30)
    dek.debug("> " + Dekoder.toHexString(nextbytes) )

    var b1 = Dekoder.bytesToBits(nextbytes)
    b1.unshift(false)

    var decoded = dek.decodeManchester2(b1)
    dek.debug("> " + Dekoder.toHexString(decoded) )
