...
bool plugin004Rx(const Plugin *ps_plugin, RawSignal *ps_rawSignal, char *pc_cmd) {
   (void)(pc_cmd);
   ...
   i = 2;     // skip start bit
   b_dimPresent = false;
   (void) b_dimPresent; // Fixme : unused
   ...
   o_usart.printf("20;%02X;%s;ID=%08x;SWITCH=%x;CMD=%s;\n",
         i_sequenceNumber++, ps_plugin->pc_name, ((u32_bitStream) >> 6),
...
bool plugin004Tx(const Plugin *ps_plugin, RawSignal *ps_rawSignal, char *pc_cmd) {
   (void)(ps_plugin);
