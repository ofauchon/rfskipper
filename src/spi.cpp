...
/*----------------------------------------------------------------------------*/
/*
 * When a master is communicating with SPI slaves which need to be de-selected
 * between transmissions, the NSS pin must be configured as GPIO or another GPIO
 * must be used and toggled by software.
 */
...
void SPI::init(SPI_BASE o_spi, SPI_ChipSelect pf_chipSelect) {
  ...
  spi_init_master(
      o_spi, SPI_CR1_BAUDRATE_FPCLK_DIV_4, SPI_CR1_CPOL_CLK_TO_0_WHEN_IDLE,
