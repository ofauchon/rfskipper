...
void RFM69::setNetworkId(uint32_t u32_networkId) {
   ...
   writeRegister(REG_SYNCCONFIG, RF_SYNC_ON | RF_SYNC_FIFOFILL_AUTO |
                                 RF_SYNC_TOL_0 | (i << 3));
   for (; i > 0; i--) {
...
//         vTaskDelay(pdMS_TO_TICKS(rand() % 10));
         ...
         if (i_bytesRead > 0) {
            ...
            i_loop = 0;
            while (((readRegister(REG_RSSICONFIG) & RF_RSSI_DONE)
                  == 0) && i_loop++ < 10) {
//               vTaskDelay(pdMS_TO_TICKS(1));
               asm("nop");
            }
...
void RFM69::setDataMode(RFM69DataMode e_dataMode) {
   ...
   case RFM69_DATA_MODE_PACKET:
      writeRegister(REG_DATAMODUL, u8_value);
      writeRegister(REG_DIOMAPPING1, RF_DIOMAPPING1_DIO0_00); // Dio0Mapping = 00 (DIO0 is "Packet Sent")
      break;

   case RFM69_DATA_MODE_CONTINUOUS_WITH_SYNC:
      writeRegister(REG_DATAMODUL, u8_value | 0x40);
      writeRegister(REG_DIOMAPPING1, RF_DIOMAPPING1_DIO2_01); // Dio2Mapping = 01 (Data)
//      continuousBit(false);
      break;
