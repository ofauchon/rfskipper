...
extern RING<60> o_usb1In;
extern RING<60> o_usb2In;
extern RING<60> o_usb3In;
...
extern "C" void tim3_isr(void)
{
   ...
   if (timer_get_flag(TIM3, TIM_SR_CC3IF) != 0)
   {
      ...
      {
         if (s_rawSignal.pu16_pulse - s_rawSignal.pu16_pulses >= s_rawSignal.u16_pulses)
         {
   ...
   if (memcmp(pc_commandLine, "10;", 3) == 0)
   {
      ...
      else
      {
         ...
         for (uint i = 0; i < u8_plugins; i++)
         {
            ...
            {
               if (ps_plugin->pf_txCore != NULL && ps_plugin->pf_txCore(ps_plugin, &s_rawSignal, pc_src))
               {
