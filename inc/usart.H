...
public:
   void init(USART_BASE o_usart, uint32_t u32_baudRate = 57600,
             uint32_t u32_dataBits = 8, uint32_t u32_parity =
             USART_PARITY_NONE,
             uint32_t u32_stopBits = USART_STOPBITS_1, uint32_t u32_mode =
                   USART_MODE_TX_RX,
             uint32_t u32_flowControl =
             USART_FLOWCONTROL_NONE);

   inline void enable() {
      usart_enable(_o_usart);
   }
   inline void disable() {
      usart_disable(_o_usart);
   }
   ...
   inline void setMode(uint32_t u32_mode) {
      usart_set_mode(_o_usart, u32_mode);
   }
   ...
   inline uint16_t receive() {
      return usart_recv(_o_usart);
   }
   inline void enableRxInterrupt() {
      usart_enable_rx_interrupt(_o_usart);
   }
   inline void enableTxInterrupt() {
      usart_enable_tx_interrupt(_o_usart);
   }
   inline void enableErrorInterrupt() {
      usart_enable_error_interrupt(_o_usart);
   }
   inline void disableRxInterrupt() {
      usart_disable_rx_interrupt(_o_usart);
   }
   inline void disableTxInterrupt() {
      usart_disable_tx_interrupt(_o_usart);
   }
