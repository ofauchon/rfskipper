...
public:
   ...
   inline void cleanDisable() {
      spi_clean_disable(_o_spi);
   }

   inline void setMasterMode() {
      spi_set_master_mode(_o_spi);
   }

   inline void setSlaveMode() {
      spi_set_slave_mode(_o_spi);
   }

   inline void setFullDuplexMode() {
      spi_set_full_duplex_mode(_o_spi);
   }

   inline void setReceiveOnlyMode() {
      spi_set_receive_only_mode(_o_spi);
   }

   inline void setUnidirectionalMode() {
      spi_set_unidirectional_mode(_o_spi);
   }

   inline void setBidirectionalMode() {
      spi_set_bidirectional_mode(_o_spi);
   }

   inline void setClockPolarity0() {
      spi_set_clock_polarity_0(_o_spi);
   }

   inline void setClockPolarity1() {
      spi_set_clock_polarity_1(_o_spi);
   }

   inline void setClockPhase0() {
      spi_set_clock_phase_0(_o_spi);
   }

   inline void setClockPhase1() {
      spi_set_clock_phase_1(_o_spi);
   }
   ...
   inline void setSendLsbFirst() {
      spi_send_lsb_first(_o_spi);
   }

   inline void setSendMsbFirst() {
      spi_send_msb_first(_o_spi);
   }

   inline void enableCrc() {
      spi_enable_crc(_o_spi);
   }

   inline void disableCrc() {
      spi_disable_crc(_o_spi);
   }

   inline void setDff8bit() {
      spi_set_dff_8bit(_o_spi);
   }

   inline void setDff16bit() {
      spi_set_dff_16bit(_o_spi);
   }
   ...
   inline void enableErrorInterrupt() {
      spi_enable_error_interrupt(_o_spi);
   }

   inline void disableErrorInterrupt() {
      spi_disable_error_interrupt(_o_spi);
   }
   ...
   inline void sendData(uint16_t u16_data) {
      spi_send(_o_spi, u16_data);
   }
   ...
   inline uint16_t recvData() {
      return spi_read(_o_spi);
   }
